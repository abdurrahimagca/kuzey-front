(function ($) {
    ...
    return {
      ...
      ajaxSlider: function ($e) {
        let active = $e.parents(".slide-content"),
          ...
          img = active
            .parents(".dsn-slider")
            .find('.slide-item[data-dsn-id="' + id + '"] .image-bg')
            .first(),
      ...
      ajaxNextProject: function ($e) {
        const active = $e.parents(".next-project"),
          img = active.find(".bg-container"),
          title = $e,
          effectS = window.Scrollbar.get(
      ...
      ajaxWorkSwiper: function ($e) {
        let active = $e.parents(".content-item"),
          title = active.find(".sec-title"),
          img = active
      ...
      completeElement: function (elAjax) {
        ...
        if (title_move.length) {
          ...
          if (!titleFill.find(".dsn-chars-wrapper").length)
            dsnGrid.convertTextLine(titleFill);
          ...
          if (titleFill.length) colorFill = "var(--heading-color)";
      ...
      nextProject: function () {
        ...
        if (!dsnGrid.isMobile()) {
          ...
          parallaxIt.on("progress", function ($s) {
            if ($s.progress > 0.998) {
              if (_next.find("a").length) _next.find("a")[0].click();
            }
          });
        ...
        return new Swiper(dsn_slider.find(".slide-inner").get(0), {
          ...
          pagination: {
            ...
            renderCustom: function (swiper, current, total) {
              gsap.to(
                dsn_slider.find(".progress-nav .progress-w .progress-w-affter"),
                1,
  ...
  function Isotope() {
    $(".dsn-isotope").each(($key, $item) => {
      setTimeout(function () {
        ...
        if ($args.space)
          dsnGrid
            .convertToJQuery($item)
            .attr("style", "--gutter:" + $args.space + "px;");
  ...
  function initMap() {
    ...
    if (!mapScriptId.length) {
      let GOOGLE_MAP_KEY = "{{GOOGLE_MAPS_API_KEY}}";
